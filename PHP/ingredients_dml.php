...
function ingredients_insert(){
	...
	if(function_exists('ingredients_after_insert')){
		...
		$args=array();
		if(!ingredients_after_insert($data, getMemberInfo(), $args)){ return $recID; }
	}

	// mm: save ownership data
	sql("insert ignore into membership_userrecords set tableName='ingredients', pkValue='" . makeSafe($recID, false) . "', memberID='" . makeSafe(getLoggedMemberID(), false) . "', dateAdded='" . time() . "', dateUpdated='" . time() . "', groupID='" . getLoggedGroupID() . "'", $eo);
...
function ingredients_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if($selected_id){
		...
		if(!($row = db_fetch_array($res))){
			return error_message($Translation['No records found'], 'ingredients_view.php', false);
		}
	...
	if($dvprint){
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/ingredients_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	}else{
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/ingredients_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode=str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return ingredients_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	if($selected_id){
		...
		$templateCode=str_replace('<%%VALUE(PricingUnit)%%>', html_attr($row['PricingUnit']), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(PricingUnit)%%>', urlencode($urow['PricingUnit']), $templateCode);
		$templateCode=str_replace('<%%VALUE(RecipeUnit)%%>', html_attr($row['RecipeUnit']), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(RecipeUnit)%%>', urlencode($urow['RecipeUnit']), $templateCode);
		$templateCode=str_replace('<%%VALUE(PluralForm)%%>', html_attr($row['PluralForm']), $templateCode);
	...
	$rdata = $jdata = get_defaults('ingredients');
	if($selected_id){
		$jdata = get_joined_record('ingredients', $selected_id);
		$rdata = $row;
	}
	$cache_data = array(
	...
	$templateCode .= loadView('ingredients-ajax-cache', $cache_data);
