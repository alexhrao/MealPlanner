<?php
	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(   
			'mealdates' => array('Date Planner', '', 'resources/table_icons/application_form_magnify.png', 'Meal Planner'),
			'meals' => array('Meals', '', 'resources/table_icons/application_view_gallery.png', 'Meal Creation Suite'),
			'recipes' => array('Recipes', '', 'resources/table_icons/application_from_storage.png', 'Meal Creation Suite'),
			'ingredients' => array('Ingredients', '', 'resources/table_icons/apple.png', 'Meal Creation Suite'),
			'mealrecipes' => array('Meals & Recipes', '', 'resources/table_icons/application_form_magnify.png', 'Meal Planner'),
			'recipeingredients' => array('Recipes & Ingredients', '', 'resources/table_icons/cheque.png', 'Meal Planner'),
			'ingredientstores' => array('Ingredients & Stores', '', 'resources/table_icons/cart.png', 'Meal Planner'),
			'sources' => array('Sources', '', 'resources/table_icons/ceo.png', 'Sources'),
			'stores' => array('Stores', '', 'resources/table_icons/application_home.png', 'Sources')
		);
	...
	function get_table_groups($skip_authentication = false){
		...
		$all_groups = array('Meal Planner', 'Meal Creation Suite', 'Sources');
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			...
			'recipes' => "`recipes`.`RecipeID` as 'RecipeID', `recipes`.`Name` as 'Name', if(`recipes`.`DateCreated`,date_format(`recipes`.`DateCreated`,'%m/%d/%Y'),'') as 'DateCreated', `recipes`.`Instructions` as 'Instructions', `recipes`.`Description` as 'Description', IF(    CHAR_LENGTH(`sources1`.`FullName`), CONCAT_WS('',   `sources1`.`FullName`), '') as 'SourceID', `recipes`.`PrepTime` as 'PrepTime', `recipes`.`Servings` as 'Servings'",
			...
			'ingredientstores' => "`ingredientstores`.`IngredientStoreID` as 'IngredientStoreID', IF(    CHAR_LENGTH(`ingredients1`.`Name`) || CHAR_LENGTH(`ingredients1`.`PricingUnit`), CONCAT_WS('',   `ingredients1`.`Name`, ' - ', `ingredients1`.`PricingUnit`), '') as 'IngredientID', IF(    CHAR_LENGTH(`stores1`.`Name`), CONCAT_WS('',   `stores1`.`Name`), '') as 'StoreID', CONCAT('$', FORMAT(`ingredientstores`.`Cost`, 2)) as 'Cost'",
			'sources' => "`sources`.`SourceID` as 'SourceID', `sources`.`FullName` as 'FullName', CONCAT_WS('-', LEFT(`sources`.`PhoneNumber`,3), MID(`sources`.`PhoneNumber`,4,3), RIGHT(`sources`.`PhoneNumber`,4)) as 'PhoneNumber', `sources`.`Description` as 'Description'",
			'stores' => "`stores`.`StoreID` as 'StoreID', `stores`.`Name` as 'Name', CONCAT_WS('-', LEFT(`stores`.`PhoneNumber`,3), MID(`stores`.`PhoneNumber`,4,3), RIGHT(`stores`.`PhoneNumber`,4)) as 'PhoneNumber', `stores`.`Location` as 'Location', `stores`.`Notes` as 'Notes'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			'mealdates' => "`mealdates` LEFT JOIN `meals` as meals1 ON `meals1`.`MealID`=`mealdates`.`MealID` ",
			'meals' => "`meals` ",
			'recipes' => "`recipes` LEFT JOIN `sources` as sources1 ON `sources1`.`SourceID`=`recipes`.`SourceID` ",
			'ingredients' => "`ingredients` ",
			'mealrecipes' => "`mealrecipes` LEFT JOIN `meals` as meals1 ON `meals1`.`MealID`=`mealrecipes`.`MealID` LEFT JOIN `recipes` as recipes1 ON `recipes1`.`RecipeID`=`mealrecipes`.`RecipeID` ",
			'recipeingredients' => "`recipeingredients` LEFT JOIN `recipes` as recipes1 ON `recipes1`.`RecipeID`=`recipeingredients`.`RecipeID` LEFT JOIN `ingredients` as ingredients1 ON `ingredients1`.`IngredientID`=`recipeingredients`.`IngredientID` ",
			'ingredientstores' => "`ingredientstores` LEFT JOIN `ingredients` as ingredients1 ON `ingredients1`.`IngredientID`=`ingredientstores`.`IngredientID` LEFT JOIN `stores` as stores1 ON `stores1`.`StoreID`=`ingredientstores`.`StoreID` ",
			'sources' => "`sources` ",
			'stores' => "`stores` "
		);

		$pkey = array(   
			'mealdates' => 'MealDateID',
			'meals' => 'MealID',
			'recipes' => 'RecipeID',
			'ingredients' => 'IngredientID',
			'mealrecipes' => 'MealRecipeID',
			'recipeingredients' => 'RecipeIngredientID',
			'ingredientstores' => 'IngredientStoreID',
			'sources' => 'SourceID',
			'stores' => 'StoreID'
		);
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('Meal_Planner_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('Meal_Planner_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['Meal_Planner_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['Meal_Planner_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> Meal Planner</a>
			</div>
