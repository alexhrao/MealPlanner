	...
	/*************************************/
		...
		$pcConfig = array(
			...
			'mealrecipes' => array(   
				'MealID' => array(   
					'parent-table' => 'meals',
					'parent-primary-key' => 'MealID',
					'child-primary-key' => 'MealRecipeID',
					'child-primary-key-index' => 0,
					'tab-label' => 'Meals & Recipes',
					'auto-close' => false,
					'display-refresh' => true,
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => array(1 => 'Meal', 2 => 'Recipe'),
					'display-field-names' => array(1 => 'MealID', 2 => 'RecipeID'),
					'sortable-fields' => array(0 => '`mealrecipes`.`MealRecipeID`', 1 => '`mealrecipes`.`MealID`', 2 => '`mealrecipes`.`RecipeID`'),
					'records-per-page' => 10,
					...
					'template' => 'children-mealrecipes',
					'template-printable' => 'children-mealrecipes-printable',
					'query' => "SELECT `mealrecipes`.`MealRecipeID` as 'MealRecipeID', IF(    CHAR_LENGTH(`meals1`.`Name`), CONCAT_WS('',   `meals1`.`Name`), '') as 'MealID', IF(    CHAR_LENGTH(`recipes1`.`Name`), CONCAT_WS('',   `recipes1`.`Name`), '') as 'RecipeID' FROM `mealrecipes` LEFT JOIN `meals` as meals1 ON `meals1`.`MealID`=`mealrecipes`.`MealID` LEFT JOIN `recipes` as recipes1 ON `recipes1`.`RecipeID`=`mealrecipes`.`RecipeID` "
				)
			),
			'recipeingredients' => array(   
				'RecipeID' => array(   
					'parent-table' => 'recipes',
					'parent-primary-key' => 'RecipeID',
					'child-primary-key' => 'RecipeIngredientID',
					'child-primary-key-index' => 0,
					...
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => array(1 => 'Recipe', 2 => 'Ingredient', 3 => 'Amount'),
					'display-field-names' => array(1 => 'RecipeID', 2 => 'IngredientID', 3 => 'Amount'),
					'sortable-fields' => array(0 => '`recipeingredients`.`RecipeIngredientID`', 1 => '`recipeingredients`.`RecipeID`', 2 => '`recipeingredients`.`IngredientID`', 3 => '`recipeingredients`.`Amount`'),
					'records-per-page' => 10,
					'default-sort-by' => false,
					'default-sort-direction' => 'asc',
					'open-detail-view-on-click' => true,
					'display-page-selector' => true,
					'show-page-progress' => true,
					'template' => 'children-recipeingredients',
					'template-printable' => 'children-recipeingredients-printable',
					'query' => "SELECT `recipeingredients`.`RecipeIngredientID` as 'RecipeIngredientID', IF(    CHAR_LENGTH(`recipes1`.`Name`), CONCAT_WS('',   `recipes1`.`Name`), '') as 'RecipeID', IF(    CHAR_LENGTH(`ingredients1`.`Name`), CONCAT_WS('',   `ingredients1`.`Name`), '') as 'IngredientID', `recipeingredients`.`Amount` as 'Amount' FROM `recipeingredients` LEFT JOIN `recipes` as recipes1 ON `recipes1`.`RecipeID`=`recipeingredients`.`RecipeID` LEFT JOIN `ingredients` as ingredients1 ON `ingredients1`.`IngredientID`=`recipeingredients`.`IngredientID` "
				)
			),
			'ingredientstores' => array(   
				'IngredientID' => array(   
					'parent-table' => 'ingredients',
					'parent-primary-key' => 'IngredientID',
					'child-primary-key' => 'IngredientStoreID',
					'child-primary-key-index' => 0,
					'tab-label' => 'Stores',
					'auto-close' => false,
					'display-refresh' => true,
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => array(1 => 'Ingredient', 2 => 'Store', 3 => 'Cost'),
					'display-field-names' => array(1 => 'IngredientID', 2 => 'StoreID', 3 => 'Cost'),
					'sortable-fields' => array(0 => '`ingredientstores`.`IngredientStoreID`', 1 => '`ingredientstores`.`IngredientID`', 2 => '`ingredientstores`.`StoreID`', 3 => '`ingredientstores`.`Cost`'),
					'records-per-page' => 10,
					'default-sort-by' => false,
					'default-sort-direction' => 'asc',
					'open-detail-view-on-click' => true,
					'display-page-selector' => true,
					'show-page-progress' => true,
					'template' => 'children-ingredientstores',
					'template-printable' => 'children-ingredientstores-printable',
					'query' => "SELECT `ingredientstores`.`IngredientStoreID` as 'IngredientStoreID', IF(    CHAR_LENGTH(`ingredients1`.`Name`) || CHAR_LENGTH(`ingredients1`.`PricingUnit`), CONCAT_WS('',   `ingredients1`.`Name`, ' - ', `ingredients1`.`PricingUnit`), '') as 'IngredientID', IF(    CHAR_LENGTH(`stores1`.`Name`), CONCAT_WS('',   `stores1`.`Name`), '') as 'StoreID', CONCAT('$', FORMAT(`ingredientstores`.`Cost`, 2)) as 'Cost' FROM `ingredientstores` LEFT JOIN `ingredients` as ingredients1 ON `ingredients1`.`IngredientID`=`ingredientstores`.`IngredientID` LEFT JOIN `stores` as stores1 ON `stores1`.`StoreID`=`ingredientstores`.`StoreID` "
				)
			),
			'sources' => array(   
			),
